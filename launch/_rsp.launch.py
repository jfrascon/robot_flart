...
def generate_launch_description() -> LaunchDescription:
    ...
    used as the user entry point for the `robot_forki3` package.
    ...
    ldes: List[LaunchDescriptionEntity] = [
        ...
        DeclareLaunchArgument('robot_name', default_value='forki3', description='The unique name for the robot'),
...
def _build_xacro_command(ctx: LaunchContext) -> Tuple[List[Any], List[str]]:
    ...
    xacro_file = os.path.join(
        get_package_share_directory('robot_forki3'), 'urdf', 'models', f'model_{robot_model}.xacro'
    )
    ...
    for xarg_name in model_utils.get_xarg_names(robot_model):
        ...
        if xarg_name == 'sim_file':
            ...
            # selected model is used.
                ...
                value = os.path.join(
                    get_package_share_directory('robot_forki3'),
                    'config',
...
def _declare_model_launch_arguments(ctx: LaunchContext) -> List[LaunchDescriptionEntity]:
    ...
    if not model_utils.model_exists(robot_model):
        raise ValueError(
            f"Model '{robot_model}' for the 'robot_forki3' robot is not available. "
