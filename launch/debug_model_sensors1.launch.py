"""
Launch the Forki3 sensors1 model in a package-local Gazebo debug world.
...
def generate_launch_description() -> LaunchDescription:
    """Launch the Forki3 sensors1 model in a Gazebo world for debugging and inspection."""
    actions: list[LaunchDescriptionEntity] = [
        ...
        DeclareLaunchArgument(
            'robot_name', default_value='forki3', description='Unique robot name.'
        ),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            default_value='True',
            choices=['True', 'true', 'False', 'false'],
            description='Launch RViz with the Forki3 debug configuration.',
        ),
        DeclareLaunchArgument(
            'gzgui_enabled',
            default_value='True',
...
def _include_spawn_world() -> IncludeLaunchDescription:
    """Start the shared Gazebo world used to inspect the sensors1 model."""
    return IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            'world_sdf_file': PathJoinSubstitution(
                [FindPackageShare('robot_forki3'), 'worlds', 'debug_world.sdf']
            ),
            'world_sdf_string': '',
            'world_bridge_config_file': PathJoinSubstitution(
                [FindPackageShare('robot_forki3'), 'worlds', 'debug_world_bridge.yaml']
            ),
...
def _launch_rviz() -> Node:
    """Launch RViz with the package-local Forki3 debug configuration."""
    return Node(
        package='rviz2',
        executable='rviz2',
        name='rviz2',
