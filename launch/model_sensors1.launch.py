...
def generate_launch_description() -> LaunchDescription:
    """
    Build the launch description for the `sensors1` model of the
    `robot_forki3` package.
    """
    robot_model = 'sensors1'

    ldes: list[LaunchDescriptionEntity] = [
        SetLaunchConfiguration('robot_type', 'forki3'),
        ...
        DeclareLaunchArgument('robot_name', default_value='forki3', description="Robot's name"),
        OpaqueFunction(function=rlh.set_robot_namespace),
        OpaqueFunction(function=rlh.set_robot_prefix),
        DeclareLaunchArgument(
            'params_file',
            default_value=os.path.join(
                get_package_share_directory('robot_forki3'), 'config', 'model_sensors1', 'example_params.yaml'
            ),
        ...
        DeclareLaunchArgument(
            'bridge_file',
            default_value=os.path.join(
                get_package_share_directory('robot_forki3'), 'config', 'model_sensors1', 'example_bridge.yaml'
            ),
    ...
    ldes.extend(
        [
            ...
            DeclareLaunchArgument(
                'three_swerve_kinematics_node_remappings', default_value='', description=rlh.REMAPPINGS_DESC
            ),
            ####################################################################
            # NODE OPTIONS
            ####################################################################
            ...
            _include_rsp(),
            _include_bridge(),
            _include_three_swerve_kinematics(),
        ]
    )
...
def _include_three_swerve_kinematics() -> IncludeLaunchDescription:
    """
    Include the three-swerve kinematics launch file for this model.
    """
    return IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
