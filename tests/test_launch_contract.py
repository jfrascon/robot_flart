...
def test_robot_type_context_values_identify_forki3() -> None:
    ...
    for launch_file in LAUNCH_FILES:
        ...
        for node in ast.walk(tree):
            if not (
                ...
                and isinstance(node.func, ast.Name)
                and node.func.id == 'SetLaunchConfiguration'
                and len(node.args) >= 2
                and isinstance(node.args[0], ast.Constant)
                and node.args[0].value == 'robot_type'
            ):
                continue

            assert isinstance(node.args[1], ast.Constant), launch_file
            assert node.args[1].value == 'forki3', launch_file
...
def test_model_launch_renders_one_shared_params_file_for_all_children(launch_file: Path) -> None:
    ...
    # Both Forki3 models include RSP, kinematics, and fork control.
