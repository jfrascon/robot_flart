...
@pytest.mark.parametrize('robot_model', ['base', 'sensors1'])
def test_model_params_select_the_default_node_names(robot_model: str) -> None:
    params_file = PACKAGE_DIR / 'config' / f'model_{robot_model}' / 'default_params.yaml'
    params = yaml.safe_load(params_file.read_text(encoding='utf-8'))
    configured_node_names = {selector.removeprefix('/**/') for selector in params}

    assert configured_node_names == {
...
@pytest.mark.parametrize(
    'launch_file', ['real_model_base.launch.py', 'real_model_sensors1.launch.py']
)
