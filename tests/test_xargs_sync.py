...
def test_model_exists_matches_available_robot_models() -> None:
    ...
    assert not model_utils.model_exists('m3')
...
def test_model_has_xargs_matches_current_xargs_files() -> None:
    ...
    assert not model_utils.model_has_xargs('m3')
...
def test_sensors1_xargs_match_common_and_model_xacro_args() -> None:
    merged_arg_names = (
        _xacro_arg_names(PACKAGE_DIR / 'urdf' / 'includes' / 'common.xacro')
        | _xacro_arg_names(PACKAGE_DIR / 'urdf' / 'models' / 'model_sensors1.xacro')
    ) - RESERVED_LAUNCH_ARGS

    assert set(model_utils.get_xarg_names('sensors1')) == merged_arg_names
...
def test_sensors1_xargs_defaults_match_xacro_defaults() -> None:
    common_defaults = _xacro_arg_defaults(PACKAGE_DIR / 'urdf' / 'includes' / 'common.xacro')
    model_defaults = _xacro_arg_defaults(PACKAGE_DIR / 'urdf' / 'models' / 'model_sensors1.xacro')
    merged_defaults = {
        **{k: v for k, v in common_defaults.items() if k not in RESERVED_LAUNCH_ARGS},
        **{k: v for k, v in model_defaults.items() if k not in RESERVED_LAUNCH_ARGS},
    }

    xargs = model_utils._get_xargs('sensors1')
