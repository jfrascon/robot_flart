...
def generate_launch_description():
    ...
    ldes: list[LaunchDescriptionEntity] = [
        ...
        # Other versions (v1, v1, ...) extend the core version with additional features.
        DeclareLaunchArgument(
...
def include_rosgz_bridge(ctx: LaunchContext) -> List[LaunchDescriptionEntity]:
    ...
    # Every version of the 'fs3aw' robot uses the 'core_sim_file'.
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution(
                    [FindPackageShare('robot_forklift_simple_3aw'), 'launch', f'rosgz_bridge_{robot_version}.launch.py']
                )
            ),
...
def get_launch_configurations_from_resolved_xargs(robot_version: str) -> dict[str, LaunchConfiguration]:
    return {
        xarg_name: LaunchConfiguration(xarg_name) for xarg_name in fs3aw_xargs.get_resolved_xargs(robot_version).keys()
    }
