...
def generate_launch_description():
    """Build the launch description for the FLART v1 robot profile."""
...
def include_rsp(_ctx: LaunchContext) -> List[LaunchDescriptionEntity]:
    """Include the v1-specific RSP launch file."""
    return [
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution([FindPackageShare('robot_forklift_simple_3aw'), 'launch', 'rsp_v1.launch.py'])
            ),
            launch_arguments={
...
def include_rosgz_bridge(ctx: LaunchContext) -> List[LaunchDescriptionEntity]:
    """Include the v1 rosgz bridge launch when core and extras simulation plugins are enabled."""
    ...
    # The v1 profile requires both simulation files (`core` and `extras`) to launch the bridge stack.
        ...
        IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                PathJoinSubstitution(
                    [FindPackageShare('robot_forklift_simple_3aw'), 'launch', 'rosgz_bridge_v1.launch.py']
                )
            ),
            launch_arguments={
                'use_sim_time': LaunchConfiguration('use_sim_time'),
