...
def generate_launch_description():
    """Build the launch description for the FLART v1 RSP profile."""
    ...
    ldes = [
        ...
        DeclareLaunchArgument('robot_name', default_value='fs3aw_v1', description='The unique name for the robot'),
        ########################################################################
        # Parameters
        ########################################################################
        DeclareLaunchArgument(
    ...
    ldes.extend(fs3aw_xargs.declare_launch_arguments_for_robot_version(_ROBOT_VERSION))
...
def launch_rsp(ctx: LaunchContext) -> List[LaunchDescriptionEntity]:
    """Launch robot_state_publisher for the FLART v1 profile."""
    ...
    available_robot_versions = fs3aw_xargs.get_robot_versions()

    if robot_version not in available_robot_versions:
        return [
            LogInfo(
                msg=f"[ERROR][{underscored_robot_ns}] Version '{robot_version}' for the 'fs3aw' robot is not "
    ...
    if not use_sim_time:
        ...
        # The <xacro:arg> 'core_sim_file' is part of the 'core' version of the fs3aw robot.
        # Every version of the fs3aw robot beyond 'core' (v1, v1, ...) includes the core version and adds additional
        ...
        if 'extras_sim_file' in fs3aw_xargs.get_resolved_xargs(robot_version):
            ...
            ldes.append(
                LogInfo(
                    msg=f"[WARNING][{underscored_robot_ns}] Version '{robot_version}' of the 'fs3aw' robot does not "
    ...
    for xarg_name in fs3aw_xargs.get_resolved_xargs(robot_version).keys():
        # Every version of the 'fs3aw' robot uses the 'core_sim_file', so the variable 'core_sim_file' is always added
        # to 'cmd'.
